package com.icell.external.carlosformito.core.ui.extensions
...
fun FormFieldValidationResult.Invalid.errorMessage(): String? {
    return when (this) {
        is FormFieldValidationResult.Invalid.MessageWithArgs -> {
            stringResource(errorMessageId, *formatArgs.toTypedArray())
        }

        else -> {
            errorMessageId?.let { id -> stringResource(id) }
        }
    }
}
